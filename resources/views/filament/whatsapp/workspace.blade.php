...
<div
    ...
    style="display:grid; grid-template-columns: calc(25% - 10.67px) calc(55% - 10.66px) calc(20% - 10.67px); gap:16px; height:calc(100vh - 7rem); min-height:0; width:100%; padding:0; box-sizing:border-box; background:#f3f4f6;"
>
    {{-- ====== Kolom 1: Threads ====== --}}
    ...
    <div style="display:flex; flex-direction:column; border:1px solid #e5e7eb; border-radius:8px; overflow:hidden; min-height:0;">
                ...
                <div style="display:flex; {{ $m['out'] ? 'justify-content:flex-end;' : 'justify-content:flex-start;' }}">
                    <div style="max-width:80%;">
                        ...
                        ">
                            ...
                            {{-- TEMPLATE --}}
                            @if($type === 'template')
                                <div style="white-space:normal; font-size:15px; line-height:normal;">
                            ...
                            @elseif(in_array($type, ['image','sticker','video','document','audio']))
                                ...
                                @if(!empty($m['text']))
                                    <div style="margin-top:4px; white-space:pre-line; font-size:15px; line-height:0.5;">
                                        {!! nl2br(e($m['text'])) !!}
                                    </div>
                                @endif
                            @endif
        ...
        <div style="border-top:1px solid #e5e7eb; background:#fff; padding:10px;">
            ...
            >
                ...
                <!-- GANTI input -> textarea. Hapus x-on:keydown.enter.prevent -->
                <textarea
                wire:model.defer="text"
                placeholder="Tulis pesan… Klik Send untuk kirim"
                rows="3"
                style="flex:1; border:1px solid #d1d5db; border-radius:8px; padding:8px 10px; font-size:14px; background:#fff; resize:vertical; white-space:pre-wrap;"
                ></textarea>
